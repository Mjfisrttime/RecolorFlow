...
export default function App() {
    ...
    const handleFileUpload = (e) => {
        ...
        // Enforce max file size of 50MB (Client-Side DoS mitigation)
        filteredFiles = filteredFiles.filter(f => f.size <= 50 * 1024 * 1024);

        // Enforce max total file count of 100
        if (files.length + filteredFiles.length > 100) {
            alert("Maximum of 100 files allowed.");
            filteredFiles = filteredFiles.slice(0, Math.max(0, 100 - files.length));
        }
                                    ...
                                    {variants.map((variant, vIdx) => (
                                        <div key={variant.id} className={`bg-surface-container border p-4 rounded-xl relative transition-all ${selectedVariantId === variant.id ? 'border-primary ring-1 ring-primary/50 shadow-lg' : 'border-outline-variant'}`}>
                                            ...
                                            <div className="mb-4">
                                                ...
                                                <input id={`variant-name-${variant.id}`} aria-label={`Name for Variant ${vIdx + 1}`} type="text" value={variant.name} maxLength={50} onChange={e => setVariants(variants.map(v => v.id === variant.id ? {...v, name: e.target.value} : v))} className="w-full bg-surface-container-lowest border border-outline-variant rounded p-1.5 text-sm text-on-surface outline-none focus:border-primary focus:ring-1 focus:ring-primary"/>
                                            </div>
                                ...
                                {showAdvanced && (
                                    <div className="mt-4 space-y-4 p-4 bg-surface-container rounded-xl border border-outline-variant">
                                        ...
                                        <div className="pt-3 border-t border-outline-variant/50">
                                            ...
                                            <input id="output-suffix" type="text" value={outputSuffix} maxLength={50} onChange={(e) => setOutputSuffix(e.target.value)} className="w-full bg-surface-container-lowest border border-outline-variant rounded p-1.5 text-xs text-on-surface focus:outline-none focus:border-outline focus-visible:ring-1 focus-visible:ring-primary"/>
                                        </div>
