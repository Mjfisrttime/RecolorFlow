...
export default function App() {
    ...
    const handleFileUpload = (e) => {
        ...
        // SECURITY ENHANCEMENT: Limit file count and size to prevent client-side DoS (memory exhaustion)
        const MAX_FILES = 50;
        ...
        if (filesToProcess.length > MAX_FILES) {
            alert(`For stability, you can only upload up to ${MAX_FILES} files at a time. The first ${MAX_FILES} files will be loaded.`);
            filesToProcess = filesToProcess.slice(0, MAX_FILES);
        }

        const oversized = filesToProcess.filter(f => f.size > MAX_SIZE_BYTES);
        if (oversized.length > 0) {
            alert(`${oversized.length} file(s) exceeded the ${MAX_SIZE_MB}MB size limit and were skipped.`);
            filesToProcess = filesToProcess.filter(f => f.size <= MAX_SIZE_BYTES);
        }

        if (filesToProcess.length > 0) {
            trackEvent('file_uploaded', { mode, file_count: filesToProcess.length });
        }
