...
describe('gifWorker utilities', () => {
    ...
    describe('rgbDistanceSq', () => {
        it('should return 0 for identical colors', () => {
            expect(rgbDistanceSq(255, 0, 0, 255, 0, 0)).toBe(0);
            expect(rgbDistanceSq(0, 255, 0, 0, 255, 0)).toBe(0);
            expect(rgbDistanceSq(0, 0, 255, 0, 0, 255)).toBe(0);
            expect(rgbDistanceSq(128, 128, 128, 128, 128, 128)).toBe(0);
        });
        ...
        it('should calculate correct distance between primary colors', () => {
            ...
            expect(rgbDistanceSq(255, 0, 0, 0, 255, 0)).toBe(130050);

            // distance between black (0,0,0) and white (255,255,255)
            // sqrt(3 * 255^2) = sqrt(195075) ~= 441.673
            expect(rgbDistanceSq(0, 0, 0, 255, 255, 255)).toBe(195075);
        });

        it('should be symmetric', () => {
            const dist1 = rgbDistanceSq(10, 20, 30, 40, 50, 60);
            const dist2 = rgbDistanceSq(40, 50, 60, 10, 20, 30);
            expect(dist1).toBe(dist2);
        });

        it('should calculate known smaller distances correctly', () => {
            // (0,0,0) to (3,4,0) -> sqrt(9 + 16) = 5
            expect(rgbDistanceSq(0, 0, 0, 3, 4, 0)).toBe(25);
        });
    });
});
