...
## 2024-05-24 - Client-Side DoS via Memory Exhaustion (File Uploads)
**Vulnerability:** The application allowed uploading an unbounded number of files of any size simultaneously, potentially leading to a Client-Side Denial of Service (DoS) due to memory exhaustion, especially when processing large images. In addition, user text inputs were not length-restricted.
**Learning:** Even entirely client-side applications can suffer from DoS if user input dictates unbound memory allocations (like processing and storing large `dataUrl`s for thousands of files).
**Prevention:** Implement strict file size and total file count limits directly in file upload handlers. Also, enforce `maxLength` restrictions on user text inputs to prevent excessive memory usage or layout-breaking behavior.
